...
> **Kids-app fork.** This branch tracks upstream releases while permanently
> disabling device advertiser-ID collection. Android starts with Facebook
> automatic logging and advertiser-ID collection off, removes AAID and Privacy
> Sandbox advertising permissions, and both native bridges refuse to enable
> advertiser-ID collection. Android and CocoaPods native dependencies are
> pinned, SwiftPM stays on Meta's 18.x line, and privacy is enforced locally so
> it does not depend on Meta's defaults.
...
Calling `setAdvertiserIdCollectionEnabled(true)` in this fork throws
`UnsupportedError`. Aggregate App Events remain available without IDFA/AAID.
